...
: base(inputStream, postModel)
        ...
        public void userRegister(RequestMessageText requestMessage)
        {
            try
            {
                //获得当前公众号
                IWC_OfficalAccountsRepository m_Rep = new WC_OfficalAccountsRepository(null);
                var OfficalAccountModel = m_Rep.Find(task => task.AppId == postModelP.AppId);
                //将用户提取到本地数据库
                IWC_UserBLL user_BLL = new WC_UserBLL()
                {
                m_Rep = new WC_UserRepository(null)
                };
                WC_UserModel userModel = new WC_UserModel();
                UserInfoJson userJson = UserApi.Info(OfficalAccountModel.AccessToken, requestMessage.FromUserName);
                if (user_BLL.IsExists(userJson.openid))
                {
                    return;
                }
                else
                {
                    userModel.Id = userJson.openid;
                    ...
                    userModel.OfficalAccountId = OfficalAccountModel.Id;
        ...
        public override IResponseMessageBase OnVoiceRequest(RequestMessageVoice requestMessage)
        {
            ...
            //获得当前公众号
            WC_OfficalAccountsModel account = account_BLL.GetById(postModelP.AppId);
