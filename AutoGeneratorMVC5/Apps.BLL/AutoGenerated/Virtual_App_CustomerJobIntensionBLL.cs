//------------------------------------------------------------------------------
// <auto-generated>
//     此代码已从模板生成。
//
//     手动更改此文件可能导致应用程序出现意外的行为。
//     如果重新生成代码，将覆盖对此文件的手动更改。
// </auto-generated>
//------------------------------------------------------------------------------

using System;
using System.Collections.Generic;
using System.Linq;
using Apps.Models;
using Apps.Common;
using Microsoft.Practices.Unity;
using System.Transactions;
using Apps.IBLL;
using Apps.IDAL;
using Apps.BLL.Core;
using Apps.Locale;
using Apps.IDAL.App;
using Apps.Models.App;
namespace Apps.BLL.App
{
	public class Virtual_App_CustomerJobIntensionBLL
	{
        [Dependency]
        public IApp_CustomerJobIntensionRepository m_Rep { get; set; }

		public virtual List<App_CustomerJobIntensionModel> GetList(ref GridPager pager, string queryStr)
        {

            IQueryable<App_CustomerJobIntension> queryData = null;
            if (!string.IsNullOrWhiteSpace(queryStr))
            {
                queryData = m_Rep.GetList(
								a=>a.Id.Contains(queryStr)
								
								
								|| a.CreateUserName.Contains(queryStr)
								|| a.ModificationUserName.Contains(queryStr)
								
								|| a.ParentId.Contains(queryStr)
								|| a.PK_App_Customer_CustomerName.Contains(queryStr)
								|| a.EnumPositionType.Contains(queryStr)
								|| a.ExpectSalary.Contains(queryStr)
								|| a.ExpectCountry.Contains(queryStr)
								|| a.Skills.Contains(queryStr)
								|| a.Intention.Contains(queryStr)
								|| a.EnumApplyWay.Contains(queryStr)
								|| a.CurrentPlace.Contains(queryStr)
								|| a.EnumForeignLangGrade.Contains(queryStr)
								|| a.AbroadExp.Contains(queryStr)
								);
            }
            else
            {
                queryData = m_Rep.GetList();
            }
            pager.totalRows = queryData.Count();
            //排序
            queryData = LinqHelper.SortingAndPaging(queryData, pager.sort, pager.order, pager.page, pager.rows);
            return CreateModelList(ref queryData);
        }
        public virtual List<App_CustomerJobIntensionModel> CreateModelList(ref IQueryable<App_CustomerJobIntension> queryData)
        {

            List<App_CustomerJobIntensionModel> modelList = (from r in queryData
                                              select new App_CustomerJobIntensionModel
                                              {
													Id = r.Id,
													CreateTime = r.CreateTime,
													ModificationTime = r.ModificationTime,
													CreateUserName = r.CreateUserName,
													ModificationUserName = r.ModificationUserName,
													SortCode = r.SortCode,
													ParentId = r.ParentId,
													PK_App_Customer_CustomerName = r.PK_App_Customer_CustomerName,
													EnumPositionType = r.EnumPositionType,
													ExpectSalary = r.ExpectSalary,
													ExpectCountry = r.ExpectCountry,
													Skills = r.Skills,
													Intention = r.Intention,
													EnumApplyWay = r.EnumApplyWay,
													CurrentPlace = r.CurrentPlace,
													EnumForeignLangGrade = r.EnumForeignLangGrade,
													AbroadExp = r.AbroadExp,
          
                                              }).ToList();

            return modelList;
        }

        public virtual bool Create(ref ValidationErrors errors, App_CustomerJobIntensionModel model)
        {
            try
            {
                App_CustomerJobIntension entity = m_Rep.GetById(model.Id);
                if (entity != null)
                {
                    errors.Add(Resource.PrimaryRepeat);
                    return false;
                }
                entity = new App_CustomerJobIntension();
               				entity.Id = model.Id;
				entity.CreateTime = model.CreateTime;
				entity.ModificationTime = model.ModificationTime;
				entity.CreateUserName = model.CreateUserName;
				entity.ModificationUserName = model.ModificationUserName;
				entity.SortCode = model.SortCode;
				entity.ParentId = model.ParentId;
				entity.PK_App_Customer_CustomerName = model.PK_App_Customer_CustomerName;
				entity.EnumPositionType = model.EnumPositionType;
				entity.ExpectSalary = model.ExpectSalary;
				entity.ExpectCountry = model.ExpectCountry;
				entity.Skills = model.Skills;
				entity.Intention = model.Intention;
				entity.EnumApplyWay = model.EnumApplyWay;
				entity.CurrentPlace = model.CurrentPlace;
				entity.EnumForeignLangGrade = model.EnumForeignLangGrade;
				entity.AbroadExp = model.AbroadExp;
  

                if (m_Rep.Create(entity))
                {
                    return true;
                }
                else
                {
                    errors.Add(Resource.InsertFail);
                    return false;
                }
            }
            catch (Exception ex)
            {
                errors.Add(ex.Message);
                ExceptionHander.WriteException(ex);
                return false;
            }
        }



         public virtual bool Delete(ref ValidationErrors errors, string id)
        {
            try
            {
                if (m_Rep.Delete(id) == 1)
                {
                    return true;
                }
                else
                {
                    return false;
                }
            }
            catch (Exception ex)
            {
                errors.Add(ex.Message);
                ExceptionHander.WriteException(ex);
                return false;
            }
        }

        public virtual bool Delete(ref ValidationErrors errors, string[] deleteCollection)
        {
            try
            {
                if (deleteCollection != null)
                {
                    using (TransactionScope transactionScope = new TransactionScope())
                    {
                        if (m_Rep.Delete(deleteCollection) == deleteCollection.Length)
                        {
                            transactionScope.Complete();
                            return true;
                        }
                        else
                        {
                            Transaction.Current.Rollback();
                            return false;
                        }
                    }
                }
                return false;
            }
            catch (Exception ex)
            {
                errors.Add(ex.Message);
                ExceptionHander.WriteException(ex);
                return false;
            }
        }

		
       

        public virtual bool Edit(ref ValidationErrors errors, App_CustomerJobIntensionModel model)
        {
            try
            {
                App_CustomerJobIntension entity = m_Rep.GetById(model.Id);
                if (entity == null)
                {
                    errors.Add(Resource.Disable);
                    return false;
                }
                              				entity.Id = model.Id;
				entity.CreateTime = model.CreateTime;
				entity.ModificationTime = model.ModificationTime;
				entity.CreateUserName = model.CreateUserName;
				entity.ModificationUserName = model.ModificationUserName;
				entity.SortCode = model.SortCode;
				entity.ParentId = model.ParentId;
				entity.PK_App_Customer_CustomerName = model.PK_App_Customer_CustomerName;
				entity.EnumPositionType = model.EnumPositionType;
				entity.ExpectSalary = model.ExpectSalary;
				entity.ExpectCountry = model.ExpectCountry;
				entity.Skills = model.Skills;
				entity.Intention = model.Intention;
				entity.EnumApplyWay = model.EnumApplyWay;
				entity.CurrentPlace = model.CurrentPlace;
				entity.EnumForeignLangGrade = model.EnumForeignLangGrade;
				entity.AbroadExp = model.AbroadExp;
 


                if (m_Rep.Edit(entity))
                {
                    return true;
                }
                else
                {
                    errors.Add(Resource.NoDataChange);
                    return false;
                }

            }
            catch (Exception ex)
            {
                errors.Add(ex.Message);
                ExceptionHander.WriteException(ex);
                return false;
            }
        }

      

        public virtual App_CustomerJobIntensionModel GetById(string id)
        {
            if (IsExists(id))
            {
                App_CustomerJobIntension entity = m_Rep.GetById(id);
                App_CustomerJobIntensionModel model = new App_CustomerJobIntensionModel();
                              				model.Id = entity.Id;
				model.CreateTime = entity.CreateTime;
				model.ModificationTime = entity.ModificationTime;
				model.CreateUserName = entity.CreateUserName;
				model.ModificationUserName = entity.ModificationUserName;
				model.SortCode = entity.SortCode;
				model.ParentId = entity.ParentId;
				model.PK_App_Customer_CustomerName = entity.PK_App_Customer_CustomerName;
				model.EnumPositionType = entity.EnumPositionType;
				model.ExpectSalary = entity.ExpectSalary;
				model.ExpectCountry = entity.ExpectCountry;
				model.Skills = entity.Skills;
				model.Intention = entity.Intention;
				model.EnumApplyWay = entity.EnumApplyWay;
				model.CurrentPlace = entity.CurrentPlace;
				model.EnumForeignLangGrade = entity.EnumForeignLangGrade;
				model.AbroadExp = entity.AbroadExp;
 
                return model;
            }
            else
            {
                return null;
            }
        }

        public virtual bool IsExists(string id)
        {
            return m_Rep.IsExist(id);
        }
		  public void Dispose()
        { 
            
        }

	}
}
