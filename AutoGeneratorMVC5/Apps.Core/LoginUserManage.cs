...
namespace Apps.Core
{
    public static class LoginUserManage
    {
            ...
            if (account != null)
            {
                ...
                if (siteConfig.issinglelogin == 1)
                {
                    if (IsChange(HttpContext.Current.Session.SessionID, account.Id))
                    {
                        //同一帐号已经在其他机子登录
                        bResult = true;
                        RedirectUrl("同一帐号已经在其他机器登录", null);
                    }
                }
            }
            return bResult;
        ...
        public static void RedirectUrl(string message, string url)
        {
            ...
            else
            {
                ...
                //HttpContext.Current.Server.Execute(url);
                //HttpContext.Current.Response.Redirect(url);
                HttpContext.Current.Response.Write("<script>   top.window.location.reload();</script>");
            }
        ...
        {
            //解密Ticket
            var Ticket = DecryptToken(encryptToken.Trim());
            //取得session，不通过说明用户退出，或者session已经过期
            var token = HttpContext.Current.Session[Ticket];
            if (token == null)
            {
                return false;
            }
        ...
        //========================================WebAPI Token================================
        public static string GetToken(WebApiOnlineUser LoginAccount)
        {
            string AccountKey = LoginAccount.AccountId;
            string Tokenkey = ResultHelper.NewId;
            WebApiOnlineUser value = LoginAccount;
            CacheHelper.Insert(Tokenkey, AccountKey, 60 * 24 * 30);
            LoginAccount.TokenKey = Tokenkey;
            CacheHelper.Insert(AccountKey, value, 60*24*30);
            var Token = AESEncryptHelper.AESEncrypt(Tokenkey);
            return Token;
        }
        public static void RemoveOnlineAccount(string Token)
        {
            try
            {
                string Tokenkey = AESEncryptHelper.AESDecrypt(Token);
                string AccountKey = CacheHelper.Get(Tokenkey).ToString();
                CacheHelper.Remove(Tokenkey);
                CacheHelper.Remove(AccountKey);
            }
            catch (Exception ex)
            {
                return;
        ...
        public static WebApiOnlineUser GetOnlineAccountToken(string Token)
        {
            try
            {
                string Tokenkey = AESEncryptHelper.AESDecrypt(Token);
                string AccountKey = CacheHelper.Get(Tokenkey).ToString();
                WebApiOnlineUser _WebApiOnlineUser = CacheHelper.Get<WebApiOnlineUser>(AccountKey);
                if (_WebApiOnlineUser.TokenKey == Tokenkey)
                {
                    return _WebApiOnlineUser;
                }
                else
                {
                    CacheHelper.Remove(Tokenkey);
