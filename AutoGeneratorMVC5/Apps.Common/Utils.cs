...
namespace Apps.Common
{
    public class Utils
    {
        ...
        public static string ObjectToDateTimeStr(object obj, string formatter)
        {
            ...
            {
                return StrToDateTime(obj.ToString()).ToString(formatter);
            }
        }
        ...
        {
            if ((expression == null) || (expression.Length > 12))
                return defValue;
        ...
        #region 获取富文本里的图片路径集合
        /// <summary>
        /// 获取富文本里的图片路径集合
        /// </summary>
        /// <param name="Content">富文本内容</param>
        /// <param name="Length">需要返回几张图片</param>
        /// <returns></returns>
        public static List<string> GetPhotoList(string Content, int Length)
        {
            var PhotoList = new List<string>();
            var iCount = 0;
            //拆分出来富文本里的图片
            // 例：<p><img src="/upload/image/xxxx/xxxx.jpg" title="xxxx" alt="xxxx"/></p>
            if (!string.IsNullOrEmpty(Content) && Content.IndexOf("<img src=\"") >= 0)
            {
                string[] imgArray = Regex.Split(Content, "<img", RegexOptions.IgnoreCase);
                foreach (var imgStr in imgArray)
                {
                    if (imgStr.IndexOf("src=\"") != -1)
                    {
                        var temp = imgStr.Substring(imgStr.IndexOf("src=\"") + 5);
                        if (temp.IndexOf("\"") > 0 && iCount < Length)
                        {
                            var CustomLink = temp.Substring(0, temp.IndexOf("\""));
                            PhotoList.Add(CustomLink);
                            iCount++;
                        }
                    }
                }
        ...
        /// <returns></returns>
        public static double GetDistance(double starLongitude, double starLatitude, double endLongitude, double endLatitude)
        {
            double lon1 = (Math.PI / 180) * starLongitude;
            double lon2 = (Math.PI / 180) * endLongitude;
            double lat1 = (Math.PI / 180) * starLatitude;
        ...
        /// <returns></returns>
        public static string GetPhotoExt(string Path)
        {
            string PhotoExt = "";
            if (!string.IsNullOrEmpty(Path))
            {
                PhotoExt = Path.Substring(Path.LastIndexOf('.'));
            }
            return PhotoExt;
        }
        #endregion
