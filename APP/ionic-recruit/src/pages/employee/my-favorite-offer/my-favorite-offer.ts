...
@Component({
  selector: 'page-my-favorite-offer',
  templateUrl: 'my-favorite-offer.html'
})
export class MyFavoriteOfferPage {
  ...
  searchRequirementList(event?) {
    ...
    this._EmployeeService.getRequirementCollections(postBody).subscribe((data: any) => {
      if (data.Code == 200) {
        this.searched = true;
        if (this.pageNum == 1) {
          this.data = data.Data;
